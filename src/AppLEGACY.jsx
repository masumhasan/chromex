...
`.trim());
  ...
  // - "variation" maps to msgVarEnabled=true (background already uses it)
  ...
  // ✅ Validation UI state (only for msgVarPattern)
  ...
  // ✅ Numeric field that NEVER causes popup scroll-jitter
  // - NO type="number" (that’s the main popup scroll demon)
  // - clamps on blur
  // - filters to digits only while typing
  ...
  const handleSave = async () => {
    ...
    // Map UI mode -> stored keys (background.js compatible)
    const nextVarEnabled = uiLenMode === "variation";
    const nextLenMode = uiLenMode === "variation" ? "range" : uiLenMode; // background ignores mode when variation enabled anyway
  ...
  // ✅ UI preview: show ranges like 80–100, and variation ranges
        ...
        {showDB ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              ...
              {/* ✅ Message Length + Variation merged */}
              <Card title="Message Length">
                <div className="flex flex-col gap-3">
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        // map UI mode -> storage behavior
                        if (next === "variation") {
                          setMsgVarEnabled(true);
                          // keep msgLenMode as range (background ignores when variation enabled)
                          setMsgLenMode("range");
                        } else {
                  ...
                  {/* ✅ Variation lives INSIDE this dropdown now */}
                  {uiLenMode === "variation" && (
