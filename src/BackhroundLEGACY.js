...
async function gptChat() {
  ...
  const activeApiKey = isGrok ? grokKey : openAiKey;
...
const firstMessage =
  ...
  Use this to determine if its the first time messaging, and if so start with basic get to know question. Carefully refer to the history (if it exists) to keep the conversation flow natural.
   ...
   In slovenian, You must be able to tell the difference between a real name and a nickname `
  ...
  const restrictedSubstrings = ["nano", "o1", "o3", "o4", "gpt-5", "gpt-4.1"];
  const isRestrictedModel = restrictedSubstrings.some((sub) =>
    selectedModel.includes(sub)
  );
  ...
  const reply = choice?.content[0]?.text || chatCompletion.output_text;
  ...
  if (refusal) {
    ...
    return { success: false, message: "", error: `GPT Refusal: ${refusal}` };
  }
